...
void matrixMultiplication(const float* a, const float* b, float* c, size_t l, size_t m, size_t n, Matrix::MulMode mode);

const size_t WARP_SIZE = 32;

void printMatrix(const float* a, size_t l, size_t h)
{
...
Matrix::Matrix(const Matrix& other) : Matrix(other.m_h, other.m_w)
{
...
Matrix Matrix::mul(const Matrix& other, MulMode mode) const
{
...
__global__ void simpleMatMul(const float* a, const float* b, float* c, size_t l, size_t m, size_t n)
{
...
__global__ void sharedMatMul(const float* a, const float* b, float* c, size_t l, size_t m, size_t n)
{
    size_t cCol = blockIdx.x * blockDim.x + threadIdx.x;
    size_t cRow = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void warpIntrinsicsMatMul(const float* a, const float* b, float* c, size_t l, size_t m, size_t n)
{
    size_t cCol = blockIdx.x * blockDim.x + threadIdx.x;
    size_t cRow = blockIdx.y * blockDim.y + threadIdx.y;
    size_t tileCol = threadIdx.x;
